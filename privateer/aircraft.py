...
EDITABLE_NUMERIC_FIELDS = (
    "Year", "BaseModelYear", "MaxSpeed", "CruiseSpeed", "LtEndurance",
    ...
    "Torpedo1", "Torpedo2", "Missile2", "Carrier", "Floatplane",
    "AvailableAircraft",
)
...
def create_aircraft_type(save, nation, template_slot: int, changes: dict[str, str]) -> AircraftType:
    ...
    for key, raw in changes.items():
        ...
        if not 0 <= number <= 2**31 - 1:
            raise ValueError(f"{key} must be between 0 and 2,147,483,647")
    ...
    new_id = max(int(model.fields["Id"]) for model in types) + 1
    if new_id > 2**31 - 1:
